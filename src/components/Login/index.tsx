...
const Login = () => {
	...
	const setLogin = (userInfo) => {
		...
		typeof objInts != 'undefined' && objInts.afterPermissionCall(() => {
			...
			if (objInts.permissions.indexOf("subscribed") > -1 && objInts.permissions.indexOf("cancelled_subscription") > -1 && objInts.permissions.indexOf("can_buy_subscription") > -1) {
				permissionType = 'can_renew'
			}
			let userData = { ...userInfo, permissions: objInts.permissions, permissionType };
	...
	const handleLoginToggle = (): void => {
		...
		if (auth.isLogin) {
			...
			let loginUrl = APIS_CONFIG.login[NODE_ENV]
			console.log("Login: -> loginUrl", loginUrl);
			window.location.href = `${loginUrl}${NODE_ENV == 'development' ? `?ru=${window.location.href}` : ''}`;
		}

	}
	let firstName: string = auth.isLogin ? (auth.userInfo && auth.userInfo.firstName) : '';
	let isSubscribed: Boolean = (auth.userInfo && auth.userInfo.permissions && auth.userInfo.permissions.indexOf('subscribed') > -1) || false;
	...
	return (
		...
		<div className={styles.user}>
			<div className={styles.userName}>
				...
				{isSubscribed && <img src="https://img.etimg.com/photo/77066493.cms" className={styles.primeUserLogo} data-testId='primeUserLogo' />}
			</div>
			<div className={styles.signIn}>
				<div className={styles.userIcon}></div>
				<div onClick={handleLoginToggle} id='loginButton'>{auth.isLogin ? 'Sign out' : 'Sign In'}</div>
			</div>
		</div>
	)
}
