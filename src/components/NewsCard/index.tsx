...
const NewsCard = (props: ListProps) => {
    ...
    const handleTabClick = (type: string) => {
        dispatch(fetchCategories(query, type));
        router.push(`/topic/${query}`, `/topic/${query}/${type}`, { shallow: true });
    }
    ...
    return (
        <Fragment>
            <div className={styles.listing}>
                <ul>
                    {(cardsData?.length > 3 ? cardsData : data.data).map((item, index) => (
                        index < 3 ? renderList(item, index) : ""
                    ))}
                    ...
                    <div className={styles.tabList}>
                        {(isFetching && !loadingMoreTopic)  ? <div className={styles.loading}> <Loading /> </div> :
                            cardsData.length > 0 ? (cardsData?.slice(3).length > 0 ? cardsData?.slice(3) : cardsData).map((item, index) => renderList(item, index)) :
                                <p className={styles.noData}>No data Found</p>
                        }
                    </div>
                    {
                        cardsData?.length > 3 && <div className={styles.loadMore} onClick={loadMore}>{isFetching ? 'Loading...' : 'Load More'}</div>
                    }
                </ul>
            </div>
        </Fragment>
