...
export default function SearchList(props) {
  const { index, data, searchValue } = props;
  const searchKeyword = searchValue?.toLowerCase();
  const displayListData = (type) => {
    const listData =
      type == "primeNews"
        ? data.filter((data) => data.link.includes("/prime/"))
        : type == "news"
        ? data.filter((data) => !data.link.includes("/prime/"))
        : data;
    const list =
      ...
      listData.map((data, i) => (
        <>
          <li className={i == listData.length - 1 ? "lastList" : "list"} key={i}>
  ...
  const getCompanyData = (type) => {
    switch (type) {
      case "company":
        return data.filter(
          (data) =>
            (data.entityType == "company" && data.subType != "NonList") ||
            data.entityType == "dvr" ||
            data.entityType == "pp" ||
            data.entityType == "idr" ||
            data.entityType == "ipo"
  ...
  const displayCompanyData = (type) => {
    const compData = getCompanyData(type);
    const objLang = {
      maping: {
        HIN: "hindi",
        GUJ: "gujarati",
      ...
      currentLang: function () {
        const a = location.pathname.split("/")[1],
          pLang = getParameterByName("language");
        let currLang = "";
        for (const key in objLang.maping) {
          if (a == objLang.maping[key]) currLang = key;
        }
    ...
    const currLanguage = objLang.currentLang();
    let langPrefix = objLang.maping[currLanguage],
      langSufix = "";
    langPrefix = langPrefix ? langPrefix + "/" : "";
    langSufix = currLanguage ? ",language-" + currLanguage : "";
    const domain = "https://m.economictimes.com";
    const list =
      compData.length > 0 &&
      compData.map((data, i) => (
        <>
          <li className={i == compData.length - 1 ? "lastList" : "list"} key={i}>
            {(type == "company" || type == "companyUnlisted") && (
              <a
                href={
                  type == "company"
                    ? `${domain}/${data.tagSeoName}/stocks/companyid-${
                        data.entityType == "dvr" || data.entityType == "pp"
            ...
            {type == "crypto" && (
              <a
                href={`${domain}/markets/cryptocurrency/${escape(data.tagSeoName)}/cryptodetail/symbol-${
                  data.tagId
                }.cms`}
              >
                {makeBold(data.tagName)}
            ...
            <>
              <li className={"head"}>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  href={`${domain}/currentquote.cms?ticker=${searchKeyword}`}
                >
                  more
                </a>
            ...
            <>
              <li className={"head"}>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  href={`${domain}/commoditysearch.cms?query=${searchKeyword}`}
                >
                  more
                </a>
                <div className="clr"></div>
